...
import { Search, Filter, ChevronRight, ChevronLeft, Download, FileSpreadsheet, FileText } from "lucide-react"
...
function exportReservationsCSV(rows: ReservationRow[]) {
  const headers = ["Nom", "Téléphone", "Pack", "Statut", "Total (XAF)", "Payé (XAF)", "Restant (XAF)", "Date"]
  const lines = rows.map((r) => [
    r.nom,
    r.telephone,
    r.pack,
    r.statut,
    r.prixTotal.toString(),
    r.totalPayé.toString(),
    (r.prixTotal - r.totalPayé).toString(),
    r.dateReservation,
  ])
  const content =
    "\uFEFF" +
    [headers, ...lines]
      ...
      .join("\r\n")
  triggerDownload(new Blob([content], { type: "text/csv;charset=utf-8;" }), `reservations_mitp_${todayStr()}.csv`)
}

async function exportReservationsXLSX(rows: ReservationRow[]) {
  const XLSX = await import("xlsx")
  const data = [
    ["Nom", "Téléphone", "Pack", "Statut", "Total (XAF)", "Payé (XAF)", "Restant (XAF)", "Date"],
    ...rows.map((r) => [
      r.nom, r.telephone, r.pack, r.statut,
      r.prixTotal, r.totalPayé, r.prixTotal - r.totalPayé, r.dateReservation,
    ]),
  ]
  const ws = XLSX.utils.aoa_to_sheet(data)
  ws["!cols"] = [{ wch: 28 }, { wch: 16 }, { wch: 20 }, { wch: 16 }, { wch: 14 }, { wch: 14 }, { wch: 14 }, { wch: 16 }]
  const wb = XLSX.utils.book_new()
  XLSX.utils.book_append_sheet(wb, ws, "Réservations")
  XLSX.writeFile(wb, `reservations_mitp_${todayStr()}.xlsx`)
}
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <a
              ...
              download
              className="flex items-center gap-2 px-3 py-2 bg-secondary hover:bg-secondary/80 text-foreground rounded-md transition-colors font-medium text-sm border border-border"
            >
              <Download className="w-4 h-4" />
              CSV (serveur)
            </a>
            <button
              onClick={() => exportReservationsCSV(filteredReservations)}
              disabled={filteredReservations.length === 0}
              className="flex items-center gap-2 px-3 py-2 bg-secondary hover:bg-secondary/80 text-foreground rounded-md transition-colors font-medium text-sm border border-border disabled:opacity-40"
            >
              <FileText className="w-4 h-4 text-green-500" />
              CSV
            </button>
            <button
              onClick={() => exportReservationsXLSX(filteredReservations)}
              disabled={filteredReservations.length === 0}
              className="flex items-center gap-2 px-3 py-2 bg-secondary hover:bg-secondary/80 text-foreground rounded-md transition-colors font-medium text-sm border border-border disabled:opacity-40"
            >
