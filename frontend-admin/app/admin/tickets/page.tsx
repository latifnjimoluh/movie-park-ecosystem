...
import { Search, Filter, Eye, Download, ChevronLeft, ChevronRight, FileSpreadsheet, FileText, RefreshCw } from "lucide-react"
...
export default function TicketsPage() {
  ...
  const fetchAllTicketsForExport = async (onlyValid = false): Promise<Ticket[]> => {
    const params = new URLSearchParams({ page: "1", pageSize: "9999" })
    if (onlyValid) params.set("status", "valid")
    const res = await fetch(`${api.baseURL}/tickets?${params}`, {
  ...
  const handleExportCSV = async (onlyValid: boolean) => {
    setIsExporting(true)
    try {
      const rows = await fetchAllTicketsForExport(onlyValid)
      const headers = ["N° Ticket", "Payeur", "Téléphone", "Pack", "Statut", "Généré le"]
      const lines = rows.map((t) => [
        t.ticket_number,
        t.reservation?.payeur_name ?? "—",
        t.reservation?.payeur_phone ?? "—",
        t.reservation?.pack_name_snapshot ?? "—",
        t.status,
        new Date(t.generated_at).toLocaleDateString("fr-FR"),
      ])
      const content =
        "\uFEFF" +
        [headers, ...lines]
      ...
      triggerDownload(
        ...
        `tickets${onlyValid ? "_valides" : ""}_mitp_${todayStr()}.csv`
      )
    } catch (err: any) {
  ...
  const handleExportXLSX = async (onlyValid: boolean) => {
    setIsExporting(true)
    try {
      const rows = await fetchAllTicketsForExport(onlyValid)
      const XLSX = await import("xlsx")
      const data = [
        ["N° Ticket", "Payeur", "Téléphone", "Pack", "Statut", "Généré le"],
        ...rows.map((t) => [
          t.ticket_number,
          t.reservation?.payeur_name ?? "—",
          t.reservation?.payeur_phone ?? "—",
      ...
      ws["!cols"] = [{ wch: 20 }, { wch: 28 }, { wch: 16 }, { wch: 22 }, { wch: 12 }, { wch: 16 }]
      const wb = XLSX.utils.book_new()
      XLSX.utils.book_append_sheet(wb, ws, onlyValid ? "Tickets valides" : "Tickets")
      XLSX.writeFile(wb, `tickets${onlyValid ? "_valides" : ""}_mitp_${todayStr()}.xlsx`)
    } catch (err: any) {
      alert("Erreur export : " + err.message)
    } finally {
      setIsExporting(false)
        ...
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className="flex items-center gap-2 px-3 py-2 text-sm bg-secondary hover:bg-secondary/80 text-foreground rounded-md transition-colors border border-border disabled:opacity-40"
            >
              {isExporting ? <RefreshCw className="w-4 h-4 animate-spin" /> : <FileText className="w-4 h-4 text-green-500" />}
              CSV valides
            </button>
            <button
              ...
              className="flex items-center gap-2 px-3 py-2 text-sm bg-secondary hover:bg-secondary/80 text-foreground rounded-md transition-colors border border-border disabled:opacity-40"
            >
              {isExporting ? <RefreshCw className="w-4 h-4 animate-spin" /> : <FileSpreadsheet className="w-4 h-4 text-emerald-500" />}
              Excel valides
            </button>
            <button
              ...
              className="flex items-center gap-2 px-3 py-2 text-sm bg-secondary hover:bg-secondary/80 text-foreground rounded-md transition-colors border border-border disabled:opacity-40"
            >
              {isExporting ? <RefreshCw className="w-4 h-4 animate-spin" /> : <FileSpreadsheet className="w-4 h-4 text-blue-500" />}
              Excel tout
            </button>
          </div>
        </div>
