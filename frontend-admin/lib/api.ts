...
export const api = {
  ...
  // ---------- DONATIONS ----------
  donations: {
    getAll: () => request("GET", "/donations"),
    confirm: (id: string, transaction_id?: string, proof_url?: string) =>
      request("PATCH", `/donations/${id}/confirm`, { transaction_id, proof_url }),
    fail: (id: string) => request("PATCH", `/donations/${id}/fail`),
    uploadProof: async (id: string, file: File): Promise<{ proof_url: string }> => {
      const token = getAccessToken()
      const formData = new FormData()
      formData.append("proof", file)
      const res = await fetch(`${BASE_URL}/donations/${id}/proof`, {
        method: "POST",
        credentials: "include",
      ...
      const json = await res.json()
      if (!res.ok) throw new Error(json?.error || "Erreur upload preuve")
      return json.data || json
    },
  },
